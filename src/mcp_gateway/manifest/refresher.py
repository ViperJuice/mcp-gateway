...
def load_descriptions_cache(cache_path: Path | None = None) -> DescriptionsCache | None:
    ...
    try:
        ...
        for name, server_data in data.get("servers", {}).items():
            tools = [
                PrebuiltToolInfo(**t) for t in server_data.get("tools", [])
            ]
...
def save_descriptions_cache(cache: DescriptionsCache, cache_path: Path | None = None) -> None:
    ...
    data = {
        "# Auto-generated by mcp-gateway refresh": None,
        "# Do not edit manually - regenerate with: mcp-gateway refresh": None,
        ...
        "servers": {
            name: {
                ...
                "tools": [t.model_dump() for t in desc.tools],
            }
...
def _infer_risk(tool_name: str, description: str) -> str:
    ...
    high_risk = ["delete", "remove", "drop", "execute", "run", "write", "modify", "create", "post"]
...
async def refresh_server(
    ...
    if existing_cache and not force:
        ...
        if version and not is_version_newer(existing_cache.version, version):
            logger.debug(f"Server {server_name} is up to date (v{existing_cache.version})")
        ...
        async with stdio_client(server_params) as (read, write):
            async with ClientSession(read, write) as session:
                ...
                for tool in tools:
                    ...
                    prebuilt_tools.append(PrebuiltToolInfo(
                        name=name,
